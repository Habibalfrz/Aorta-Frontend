...
<template>
  ...
  <div class="min-h-screen bg-slate-50/50 flex flex-col relative selection:bg-primary/20 selection:text-primary">
    ...
    <header class="sticky top-0 z-40 bg-white/70 backdrop-blur-xl border-b border-slate-200/60 supports-[backdrop-filter]:bg-white/40">
      <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          ...
          <div class="flex items-center gap-6">
            ...
            <Popover>
              <PopoverTrigger as-child>
                <button class="flex items-center justify-center w-9 h-9 rounded-xl text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-colors focus:outline-none focus:ring-2 focus:ring-primary/20">
                  <LayoutGrid class="w-5 h-5" />
                </button>
              </PopoverTrigger>
              <PopoverContent class="w-80 p-5 rounded-2xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] border-slate-200/60" align="start">
                <h4 class="text-xs font-bold text-slate-400 mb-4 px-1 uppercase tracking-wider">Aplikasi Terintegrasi</h4>
                <div class="grid grid-cols-3 gap-3">
                  ...
                  <!-- SysAdmin -->
                  <button v-if="authStore.hasRole('Superadmin')" class="group flex flex-col items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                    <div class="w-12 h-12 rounded-2xl bg-slate-100 text-slate-600 flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
                      <Settings class="w-6 h-6" />
                    </div>
                    <span class="text-[11px] font-semibold text-slate-600">SysAdmin</span>
                  </button>

                  <!-- HRIS -->
                  <button v-if="authStore.hasRole('HR_Manager')" class="group flex flex-col items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                    <div class="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
                      <Users class="w-6 h-6" />
                    </div>
                    <span class="text-[11px] font-semibold text-slate-600">HRIS Core</span>
                  </button>
            ...
            <div class="flex items-center gap-2.5">
              ...
              <span class="text-base font-bold text-slate-900 tracking-tight">Aorta ESS</span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-1 sm:gap-3">
            ...
            <button class="hidden sm:flex items-center justify-center w-9 h-9 rounded-xl text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
              <Search class="w-4 h-4" />
            </button>

            <button class="relative flex items-center justify-center w-9 h-9 rounded-xl text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
              <Bell class="w-4 h-4" />
            ...
            <div class="flex items-center gap-3 pl-1">
              <div class="text-right hidden sm:block">
                <p class="text-sm font-semibold text-slate-800 leading-none">{{ authStore.user?.name || 'User' }}</p>
                <p class="text-[11px] font-medium text-slate-500 mt-1">{{ authStore.roles[0] || 'Staff' }}</p>
              </div>
              ...
              <Popover>
                ...
                <PopoverContent class="w-56 p-2 rounded-2xl border-slate-200/60 shadow-xl" align="end">
                  <div class="p-3 border-b border-slate-100 mb-2">
                    <p class="text-sm font-bold text-slate-900">{{ authStore.user?.name }}</p>
                    <p class="text-xs text-slate-500 truncate">{{ authStore.user?.email || 'user@aorta.local' }}</p>
                  </div>
                  <button class="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-600 hover:bg-slate-50 rounded-lg transition-colors">
                    <Settings class="w-4 h-4" />
                    Pengaturan Akun
                  </button>
                  <button @click="handleLogout" class="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors mt-1">
